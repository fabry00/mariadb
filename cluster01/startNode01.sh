...
docker run -d --name ${NODE_NAME} \
  ...
  -e MARIADB_EXTRA_FLAGS="--wsrep-node-incoming-address=${MYQSL_PORT};--wsrep_node_name=${NODE_NAME};--wsrep_provider_options=ist.recv_addr=${HOST}:${IST_PORT};ist.recv_bind=0.0.0.0:${IST_PORT} --wsrep_node_incoming_address=${HOST}  --wsrep_sst_receive_address=${HOST}:${SST_PORT} --wsrep_node_address=${HOST}:${GALERA_PORT}" \
